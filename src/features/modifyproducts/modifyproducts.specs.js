...
Scenario('Modificar pedido desde el carrito', () => {
	state.logins.forEach((value) => {
		mModule.Given('Dado que me encuentro en el pedido "<country>", "<user>" and "<password>" "<cuv>" modifyproducts.feature', () => {
		...
		mModule.When('Digito una cantidad de dos dígitos en el selector de cantidad Ej. <cantidad> "<cuv>"', async () => {
		...
		mModule.Then('Se actualiza el precio final del producto "<cuv>"', async () => {
		...
		mModule.Then('Se actualiza la cantidad de productos en el resumen del pedido', () => {
		...
		mModule.Then('Se actualiza el Monto Total del pedido',async () => {
